...
def upload_files(request):
    if request.method == 'POST': 
        ...
        try:
            subprocess.run(['python3', os.path.join(settings.BASE_DIR, 'insects', 'MatrixGeneration.py')], check=True)
            subprocess.run(['python3', os.path.join(settings.BASE_DIR, 'insects', 'TrainModel.py')], check=True)
            subprocess.run(['python3', os.path.join(settings.BASE_DIR, 'insects', 'infer.py')], check=True)

            # Read the output from the last script, assuming it writes to a text file
            with open(os.path.join(processed_files_dir, 'species_similarity.txt'), 'r') as output_file:
