...
from .serializers import UserRegisterSerializer, UserLoginSerializer, UserSerializer, EmailSerializer,ResetPasswordSerializer, CheckToken
...
class ResetPassword(generics.GenericAPIView):
	...
	def post(self, request, *args, **kwargs):
		...
		if (k == True):
			return response.Response({"message": "Password reset complete"},status=status.HTTP_200_OK,)
		else:
		 	return response.Response({"message": "Invalid token"},status=status.HTTP_200_OK,)
...
class TokenCheck(generics.GenericAPIView):
	...
	def post(self, request, *args, **kwargs):
		...
		k = serializer.validate(request.data)
		if (k == True):
			return response.Response({"message": "Valid token"},status=status.HTTP_200_OK,)
		else:
		 	return response.Response({"message": "Invalid token"},status=status.HTTP_200_OK,)
class PasswordReset(generics.GenericAPIView):
	...
	def post(self, request):
		...
		if user:
			...
			send_mail('FlutterForecast - Reset Password', #title
			 message, # message
			...
			return response.Response(
				{
					"message":
					f"We have sent a message to your email if such exists to reset the password"
				},
