...
function PasswordResetConfirm() {
    ...
    const { encodedPk, token } = useParams<{ encodedPk: string; token: string }>();
    ...
    if (entered != "true") {
        client.post("/userapi/check-token/" + encodedPk + "/" + token + "/", {
        }).then(function (res) {
    ...
    function submitChange(e: FormEvent) {
        ...
        if (validate() == "Valid") {
            ...
            client.post("/userapi/password-reset/" + encodedPk + "/" + token + "/", {
                password: password
            }).then(function (res) {
                // if (!entered) {
                if (errorMessage != "Password reset complete") {
    ...
    return (
        <div className="App">
            ...
            <div className="wrapperLogin">
                ...
                <div className="text2">&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;Password Confirmation</div>
                <div className="Input">
                    <img src=".\password.png" alt="" />
                    <div id="line"></div>
                    <input type="password"
                        value={passwordConfirm}
                        onChange={(e: ChangeEvent<HTMLInputElement>) => setPasswordConfirm(e.target.value)} ></input>
                </div>
                <span id="login">
