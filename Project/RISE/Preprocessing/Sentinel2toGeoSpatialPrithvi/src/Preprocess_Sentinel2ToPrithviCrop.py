...
def windowing_sentinel(filename, root_folder, year, month):
    ...
    with rasterio.open(filename) as src:
        ...
        output_filename = (
            root_folder + year + "/window_extracted_" + year + "_" + month + ".tif"
        )
...
def stacking_images_level1(root_folder, year, months):
    ...
    merged_dataset = driver.Create(
        output,
        width,
        height,
        bands * len(file_paths),
        first_dataset.GetRasterBand(1).DataType,
    )
...
def resplitting_stacks(root_folder, year):
    ...
    for t in range(time_steps):
        ...
        new_ds = gdal.GetDriverByName("GTiff").Create(
            time_step_path + f"_time_step_{t}.tif",
            image_width,
            image_height,
            bands_per_time_step,
            gdal.GDT_Float32,
...
def scaling_bands(root_folder, year):
    ...
    for time_step in range(time_steps):
        ...
        for i in range(src_data.shape[0]):  # Loop over bands
            scaled_band = np.interp(
                src_data[i], (src_min[i], src_max[i]), (tgt_min[i], tgt_max[i])
            )
...
def restack_rescaled(root_folder, year):
    ...
    print(
        "\n\nSuccessfully pre-processed and data is saved as "
        + root_folder
        + "input_"
        + year
        + ".tif \n\n"
