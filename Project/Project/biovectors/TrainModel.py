...
class RatingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return torch.tensor(self.feature_[idx], dtype=torch.long), torch.tensor(
            self.label_[idx]
        )
...
class MatrixFactorization(torch.nn.Module):
    ...
    def forward(self, pos_idxs, ys, neg_idxs, num_neg=10):
        ...
        alpha = torch.log(torch.sqrt(ys) + 1.0) + 1.0
...
train_dataloader = DataLoader(
    RatingDataset(Xs, ys), batch_size=batch_size, shuffle=True
)
...
for epoch in range(0, epochs):
    ...
    for i, (train_batch, label_batch) in pbar:
        ...
        loss = model(
            train_batch.T.to(device), label_batch.to(device), neg_idxs.to(device)
        )
        ...
        pbar.set_description(
            "training loss at {} batch {}: {}".format(epoch, i, loss.item())
        )
